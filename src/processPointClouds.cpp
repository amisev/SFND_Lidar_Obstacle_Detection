...
ProcessPointClouds<PointT>::SeparateClouds(pcl::PointIndices::Ptr inliers,
    ...
    typename pcl::PointCloud<PointT>::Ptr seg_plane(new pcl::PointCloud<PointT>);

    for (auto index: inliers->indices) {
        seg_plane->points.push_back(cloud->points[index]);
    }
    pcl::ExtractIndices<PointT> extract;
...
//    extract.filter(*obstacles);
    extract.setNegative(true);
    extract.filter(*obstacles);
    std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> segResult(obstacles,
                                                                                                      seg_plane);
    return segResult;
}
...
std::vector<std::__fs::filesystem::path> ProcessPointClouds<PointT>::streamPcd(std::string dataPath) {
    ...
    std::vector<std::__fs::filesystem::path> paths(std::__fs::filesystem::directory_iterator{dataPath},
                                                   std::__fs::filesystem::directory_iterator{});
