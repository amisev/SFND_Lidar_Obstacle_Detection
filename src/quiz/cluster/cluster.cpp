...
void proximity(size_t point_id, const std::vector<std::vector<float>> &points, std::vector<int> &cluster,
               std::vector<char> &visited, KdTree *kdtree, float distanceTol) {
    visited[point_id] = true;
    cluster.push_back(point_id);
    auto nearby_points = kdtree->search(points[point_id], distanceTol);
    for (auto np: nearby_points) {
        if (!visited[np]) {
            proximity(np, points, cluster, visited, kdtree, distanceTol);
        }
    }
}
...
euclideanCluster(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol) {
    ...
    for (int point_id = 0; point_id < points.size(); point_id++) {
        if (!visited[point_id]) {
            ...
            proximity(point_id, points, cluster, visited, tree, distanceTol);
